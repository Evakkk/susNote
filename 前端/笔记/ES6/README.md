...
- 参数扩展／展开 `...args`
    - reset 参数，收集剩余的参数，必须当到最后一个参数位置 
    - 展开数组，简写，效果和直接把数组的内容写在这儿一样
- 默认参数

```js
//reset参数，---它就是一个真正的数组，数组特有的方法都可以使用
...
let arr1 = [1, 2, 3]
let arr2 = [4, 5, 6]
let arr3 = [...arr1, ...arr2]		//数组展开写 ...arr1 == 1,2,3
console.log(arr3)
...
- [React](https://www.reactjscn.com)
    ...
    - 组件化，一个组件就是一个 class
    - JSX == bable == browser.js      ---bable转码，编译器
...
- JSON 对象
    ...
    - JSON.parse(string) ：接受一个 **JSON 字符串**并将其转换成一个 JavaScript **对象**。（解析）
    - JSON.stringify(obj) ：接受一个 JavaScript **对象**并将其转换为一个 **JSON 字符串**。（串行化）
...
* Promise是ES6引入的异步编程的新解决方案。语法上Promise是一个构造函数，用来封装异步操作并可以获取其成功或失败的结果。
  ...
  const p = new Promise(function(resolve,reject){
      setTimeout(function(){
          let data = '数据库中的数据';
          resolve(data);		//表示成功，调用这个的时候 Promise的状态-->成功
          let err = '数据读取失败';
          reject(err);
      },1000);
  ...
  //1. 如果回调函数中返回结果是非promise类型的属性，状态为成功，返回值为成功对象的值
  const result = p.then(function(value){
      //resolve,成功
      console.log(value);
      //1. 非promise类型对象
      return 123;		//1. 返回的promise对象里的值是123
      //2. 是promise对象，返回什么状态就是什么状态，该状态的值就是返回的值
...
xhr.onreadystatechange = function(){
    if(xhr.readyState===4){
        if(xhr.status>=200 && xhr.status<300){
            ...
            consle.error(xhr.status);
        }
    }
}
```

* 使用promise对ajax进行封装（jquery  $.ajax() 就是封好的,返回一个promise对象） 
...
```js
//获取一个用户，根据用户获得订单，再根据订单获得订单详细信息
const p = new Promise((resolve,reject)=>{
    fs.readFile("./file1.txt",(err,data)=>{
        resolve(data);			----p的状态为成功
    })
})
...
p.then(value => {
    return new Promise((resolve,reject)=>{
        fs.readFile("./file2.txt",(err,data)=>{
     	   resolve([value,data]);			----promise的状态为成功
    	})					
    })					//所以第一个then最后返回了 promise对象（里面的状态为成功）
}).then(value => {				//这个value就是成功的数据
    return new Promise((resolve,reject)=>{
...
- 异步和同步
    ...
    ------因为异步结果不确定什么时候返回，所以需要在回调中，写请求完成后的需要执行的代码
...
#### 12.4 先学到这     promise,Generator函数，async函数 --- 都是解决异步操作为问题
...
* [JavaScript、ES5和ES6的介绍和区别](https://blog.csdn.net/henouren/article/details/77934183)